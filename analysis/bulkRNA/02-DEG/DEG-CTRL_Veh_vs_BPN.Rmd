...
subtitle: " CTRL Condition: Vehicle vs BPN"
author: "Ximing Ran"
date: "2025-03-26"
output:
  pdf_document:
...
knitr::opts_chunk$set(
  ...
  warning = FALSE,
  message = FALSE,
  fig.path = "./results/DEG-CTRL_Veh_vs_BPN/Analysis_figure/"
)
...
result_folder_all = './results/DEG-CTRL_Veh_vs_BPN'
...
```{r volcano_DEG_0.25, fig.width=6, fig.height=6}
...
plot_volcano_plot(result_df=result_df, 
                  ...
                  file_name = "02_volcano_plot_log2fc_0.25",
                  max_p = 3,max_fc = 5,
                  thread = 0.25 ,  dot_size =2,label_gene = NULL)
```

\newpage
```{r volcano_DEG_fc_0.5, fig.width=6, fig.height=6}
plot_volcano_plot(result_df=result_df, 
                  figure_folder = file.path(result_folder,"02-DEG"),
                  file_name = "02_volcano_plot_log2fc_0.5",
                  max_p = 3,max_fc = 5,
                  thread = 0.5 ,  dot_size =2,label_gene = NULL)
...
```{r heatmap_DEG_fc_0.25, fig.width=8, fig.height=8}
# Plot the heatmap for the DEG
vsd_obj <- varianceStabilizingTransformation(dds_obj, blind = TRUE)

DEG_gene_0.25 <- result_df %>% filter(abs(log2FoldChange) > 0.25 & padj <0.05) %>% pull(GeneName)
DEG_gene_0.5 <- result_df %>% filter(abs(log2FoldChange) > 0.5 & padj <0.05) %>% pull(GeneName)
DEG_gene_1 <- result_df %>% filter(abs(log2FoldChange) > 1 & padj <0.05) %>%  pull(GeneName)

plot_gene_heatmap(vsd_obj, gene_list = DEG_gene_0.25, 
                  figure_folder = file.path(result_folder,"02-DEG"),
                  file_name = "03_heatmap_log2fc_0.25",
                  reference_group, compare_group,
...
```{r heatmap_DEG_fc_0.25_row, fig.width=8, fig.height=8}
plot_gene_heatmap(vsd_obj, gene_list = DEG_gene_0.25, 
                  figure_folder = file.path(result_folder,"02-DEG"),
                  file_name = "03_heatmap_log2fc_0.25_row",
                  reference_group, compare_group,
                  cluster_rows = TRUE, cluster_cols = FALSE, 
                  scale = "row")
```
...
```{r heatmap_DEG_fc_0.5, fig.width=8, fig.height=8}
plot_gene_heatmap(vsd_obj, gene_list = DEG_gene_0.5, 
                  figure_folder = file.path(result_folder,"02-DEG"),
                  file_name = "03_heatmap_log2fc_0.5",
                  reference_group, compare_group,
                  cluster_rows = TRUE, cluster_cols = FALSE, 
                  scale = "none")
```
...
\newpage
```{r heatmap_DEG_fc_0.5_row, fig.width=8, fig.height=8}
plot_gene_heatmap(vsd_obj, gene_list = DEG_gene_0.5, 
                  figure_folder = file.path(result_folder,"02-DEG"),
                  file_name = "03_heatmap_log2fc_0.5_row",
                  reference_group, compare_group,
                  cluster_rows = TRUE, cluster_cols = FALSE, 
                  scale = "row")
```
...
```{r Enrichment_DEG_0.25,fig.width=12, fig.height=8}
deg0.25 <- result_df %>% filter(padj < 0.05 & abs(log2FoldChange) > 0.25)
up_gene_0.25 <- deg0.25 %>% filter(log2FoldChange > 0) %>% pull(GeneName)
down_gene_0.25 <- deg0.25 %>% filter(log2FoldChange < 0) %>% pull(GeneName)

# test for the function
# if the up_gene_1.5 is empty, we will skip the analysis
if (length(up_gene_0.25) == 0) {
  print("The up_gene_0.5 is empty, skip the analysis")
} else {
  Enrichment_analysis(gene_list = up_gene_0.25,
                      result_folder = file.path(result_folder,"03-Enrichment"),
                      file_name = "02-DEG_0.25_up", gene_name_mapping, flag = "Up")
}

if (length(down_gene_0.25) == 0) {
  print("The down_gene_0.25 is empty, skip the analysis")
} else {
  Enrichment_analysis(gene_list = down_gene_0.25,
                      result_folder = file.path(result_folder,"03-Enrichment"),
                      file_name = "02-DEG_0.25_down", gene_name_mapping, flag = "Down")
}
```

\newpage
```{r Enrichment_DEG_0.5,fig.width=12, fig.height=8}
deg0.5 <- result_df %>% filter(padj < 0.05 & abs(log2FoldChange) > 0.5)
up_gene_0.5 <- deg0.5 %>% filter(log2FoldChange > 0) %>% pull(GeneName)
down_gene_0.5 <- deg0.5 %>% filter(log2FoldChange < 0) %>% pull(GeneName)

# test for the function
# if the up_gene_1.5 is empty, we will skip the analysis
if (length(up_gene_0.5) == 0) {
  print("The up_gene_0.5 is empty, skip the analysis")
} else {
  Enrichment_analysis(gene_list = up_gene_0.5,
                      result_folder = file.path(result_folder,"03-Enrichment"),
                      file_name = "02-DEG_0.5_up", gene_name_mapping, flag = "Up")
}

if (length(down_gene_0.5) == 0) {
  print("The down_gene_0.5 is empty, skip the analysis")
} else {
  Enrichment_analysis(gene_list = down_gene_0.5,
                      result_folder = file.path(result_folder,"03-Enrichment"),
                      file_name = "02-DEG_0.5_down", gene_name_mapping, flag = "Down")
}
