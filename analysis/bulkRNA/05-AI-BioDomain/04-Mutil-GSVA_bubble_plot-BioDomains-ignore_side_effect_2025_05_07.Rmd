...
knitr::opts_chunk$set(
  ...
  warning = FALSE,
  message = FALSE,
  fig.path = "./results/04-BioDomain-ignore-side_effect/Analysis_figure/"
)
...
all_result <-  read.csv(here::here("analysis", "bulkRNA" , "03-Mutil_group_GSVA" ,
                                   "results","04-Mix-effect-comb_drug" ,"mixed_model_results_all.csv"))
...
data_BAY <- data %>%
  ...
  mutate(
      ...
      combined_p_FXS_Drug_BAY < 0.05 &
      (beta_FXS * (beta_Drug_BAY + beta_FXS_Drug_BAY)) < 0 &
...
data_BP <- data %>%
  ...
  mutate(
      ...
      # full rescue: no healthy side effect + opposite‐direction FXS drug effect + non‐significant combined p
      combined_p_FXS_Drug_BP < 0.05 &
        (beta_FXS * (beta_Drug_BP + beta_FXS_Drug_BP)) < 0 &
...
data_BPN <- data %>%
  ...
  mutate(
      ...
      # full rescue: no healthy side effect + opposite‐direction FXS drug effect + non‐significant combined p
      combined_p_FXS_Drug_BPN < 0.05 &
        (beta_FXS * (beta_Drug_BPN + beta_FXS_Drug_BPN)) < 0 &
...
for(cfg in plot_configs) {
  ...
  ggsave(paste0("./results/04-BioDomain-ignore-side_effect/Bubble_", fname, ".png"),
         plot = p, width = 6, height = 7)
  ggsave(paste0("./results/04-BioDomain-ignore-side_effect/Bubble_", fname, ".pdf"),
         plot = p, width = 6, height = 7)
}
